...
<script>
    ...
    function goL(area, Line) {
        area.focus();
        var v = area.value.split('\n')
            , ch = 0;
        for (var i = 0; i < Line - 1; i++)
            ch += v[i].length;
        var o = area.createTextRange();
        o.move("character", ch);
        o.select();
    }
    function scrollToLine($textarea, lineNumber) {
        ...
        //if (lineNumber > 4) lineNumber = lineNumber - 8;
        $textarea.scrollTop(lineNumber * lineHeight);// * 100 / detectZoom()
    }
    $(function () {
        $(':button').click(function () {
            ...
            $.post('<?= Url::to('/debug/default/develop-save')?>', data, function (result) {
                if (result.status == 1) {
            ...
            var id = $(this).attr('for')
                , line = parseInt($(this).attr('line'));
            //goL(document.getElementById(id), line);
            scrollToLine($('#' + id), line);
        });
    });
</script>
</body>
