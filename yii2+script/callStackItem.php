...
<li class="<?php if ($index === 1 || !$handler->isCoreFile($file)) echo 'application'; ?> call-stack-item"
    ...
    <?php if (!empty($lines)): ?>
        <div class="code-wrap">
            ...
            <!--cqiu append 2016-5-28-->
            <?php if(!$handler->isCoreFile($file)):?>
            <textarea class="codeX" id="code_<?= (int) $index ?>" file="<?php
                if ($file !== null) echo $file; ?>" rows="10" cols="200"><?= str_replace('</textarea>', '&lt;/textarea>', file_get_contents($file))?></textarea>
            <input type="button" class="btn" for="code_<?= (int) $index ?>" line="<?php if ($line !== null) echo (int) $line; ?>" value="save" />
            <?php endif;?>
        </div>
    <?php endif; ?>
</li>
