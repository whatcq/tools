...
Picasa.prototype = {
	...
	createFrame : function() {
		...
		this.frame.innerHTML =
			...
			'<button onclick="toggleFullScreen(event)">点我全屏</button><a href="#" id="PV_Close" ><span>×</span></a>'+
			...
			'	<ul id="PV_Items"></ul>'+
			'	<div id="PV_Buttons">'+
			'		<div id="PV_Zoom"><a href="javascript: void(0);" id="PV_Btn_ZoomActual">1:1</a><a href="javascript: void(0);" id="PV_Btn_ZoomOut">-</a><a href="javascript: void(0);" id="PV_Btn_ZoomIn">+</a><a href="javascript: void(0);" id="PV_Btn_Rotate">↻↺</a></div>'+
		...
		// 按钮对象
		this.btnRotate		= getDom('PV_Btn_Rotate');		// 旋转
	...
	setElementEvent : function() {
		...
		this.btnRotate.onmouseover = function() {self.hint('左键旋转，右键反转，双击还原');}
		this.btnRotate.onmouseout = function() {self.hint();}
		this.btnRotate.onclick = function() {self.rotateAction(1); return false;}
		this.btnRotate.oncontextmenu = function() {self.rotateAction(-1); return false;}
		this.btnRotate.ondblclick = function() {self.rotateAction(0); return false;}
		//this.setQuickAction(this.btnRotate, 'self.rotateAction(1);');
	...
	// 图片旋转
	rotateAction : function(left_right) {
		if (typeof this.Picture.rotate === 'undefined'){
			this.Picture.rotate = 0;
		}
		if (left_right === 0){
			this.Picture.rotate = 0;
		}else{
			this.Picture.rotate = (this.Picture.rotate + left_right*30 + 360)%360;
		}
		this.Picture.style.transform = 'rotate('+this.Picture.rotate+'deg)';
...
function toggleFullScreen(e){
	...
	FullScreen(el.parentNode) ?el.innerHTML='退出全屏':el.innerHTML='点我全屏';
	setTimeout(function(){MyPicasa.setFrameLayout();}, 100);
