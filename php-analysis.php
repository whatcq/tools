 ...
 * ------------
 * `Apache` .htaccess
 * php_value auto_prepend_file D:\laragon\www\cqiu\tools\php-analysis.php
 * `Nginx` 在 server 或 location 块中添加以下指令：
 * fastcgi_param PHP_VALUE "auto_prepend_file=D:\laragon\www\cqiu\tools\php-analysis.php";
 * ------------
...
function _err($msg, array $traces)
{
    ...
    function _err_highlight_code($code)
    {
        ...
        return str_replace(['&lt;?php ', '&lt;?php&nbsp;'], '', highlight_string("<?php $code", TRUE));
    }
...
function _log()
{
    ...
    $now = microtime(1);
    $key = $caller['file'] . ': ' . $caller['line'] . ': ' . round($now - ($prevTime ?: $_SERVER['REQUEST_TIME_FLOAT']), 6) . 's';
