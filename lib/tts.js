...
//
// HsiaoChen (Taiwan)台湾绵女声，不区分多音字console.log(v),//;
// YunJhe (Taiwan)
// HsiaoYu (Taiwanese Mandarin)
// male|female|kid; clear|sun|soft|special
window.voices_styles = {
    'male': v => /Yunjian|Yunxi |Yunyang|YunJhe|HsiaoYu|Huihui|Kangkang/.test(v.name),
    'female': v => /Xiaoxiao|Xiaoyi|HsiaoChen|Yaoyao/.test(v.name),
...
if (speechSynthesis.onvoiceschanged !== undefined) {
    speechSynthesis.onvoiceschanged = function () {
        ...
        window.voices = speechSynthesis.getVoices().filter((v, i) => ['zh-CN', 'zh-TW'].includes(v.lang))
            .sort((a, b) => b.name.includes('Online') - a.name.includes('Online'));
        console.log(voices);
    };
}

function speak({text = "", voice = "sun", end = 0, rate = 1, pitch = 1}) {
    ...
    msg.voice = voices.find(f) || voices[0];
    msg.rate = rate; // 0.5~2
